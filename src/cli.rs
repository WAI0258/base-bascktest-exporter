...
use crate::{
    ...
    validate_replay_root, validate_stable_token_list_str, BaseNodeRpcAdapter, CatalogError,
    ContractError, ExportError, IndexerApiAdapter, IndexerMetadataProvider, JsonRpcClient,
    MetadataExportRequest, MetadataExportResult, RawExportRequest, RawExportResult,
    RawExportTotals, SourceError, StableTokenList, StateExportRequest, StateExportResult,
};
    ...
    C: JsonRpcClient,
{
    ...
    let catalog = build_resolved_pool_catalog(metadata_provider, &selected_pools, &stable_tokens)?;
...
fn print_export_summary(summary: &ExportSummary) {
    ...
    println!("resolved_pools={}", summary.resolved_pool_count);
    println!(
    ...
    println!("raw_swap_count={}", summary.raw_totals.event_counts.swap);
    println!("raw_mint_count={}", summary.raw_totals.event_counts.mint);
    println!("raw_burn_count={}", summary.raw_totals.event_counts.burn);
    println!(
        "raw_collect_count={}",
    ...
    println!("state_shard_count={}", summary.state_shard_count);
    println!(
        "state_skipped_existing_shards={}",
        summary.skipped_existing_state_shards
    );
    println!("state_repaired_shards={}", summary.repaired_state_shards);
    println!(
...
mod tests {
    ...
    use crate::{
        source::{IndexerPoolMetadata, IndexerTokenMetadata},
        IndexerMetadataProvider,
    };

    use super::{
        read_selected_pools_file, run_export_with_adapters, run_verify_command, Cli, CliError,
        ExportArgs, VerifyArgs,
    };
    ...
    impl IndexerMetadataProvider for MockIndexerProvider {
        ...
        fn fetch_token_metadata(
            &self,
            token_address: &str,
        ) -> Result<IndexerTokenMetadata, crate::SourceError> {
            self.tokens
                .get(token_address)
                .cloned()
                .ok_or_else(|| crate::SourceError::NotFound {
        ...
        fn call(&self, method: &str, params: Vec<Value>) -> Result<Value, crate::SourceError> {
            ...
            match method {
                ...
                "eth_call" => {
                    ...
                    let balance = self.balances_by_token.get(token).ok_or_else(|| {
                        crate::SourceError::InvalidRpcResponse {
                            message: format!("missing mock balance for token {token}"),
                        }
                    })?;
                    Ok(Value::String(balance.clone()))
                }
    ...
    fn build_mock_provider(addresses: &TestAddresses) -> MockIndexerProvider {
        ...
        let mut tokens = HashMap::<String, IndexerTokenMetadata>::new();
        tokens.insert(
            addresses.token_stable.clone(),
            IndexerTokenMetadata {
                address: addresses.token_stable.clone(),
                decimals: 6,
        ...
        tokens.insert(
            addresses.token_quote.clone(),
            IndexerTokenMetadata {
                address: addresses.token_quote.clone(),
                decimals: 18,
        ...
        tokens.insert(
            ...
            IndexerTokenMetadata {
                address: addresses.token_other.clone(),
                decimals: 8,
                symbol: Some("WBTC".to_owned()),
                name: Some("Wrapped BTC".to_owned()),
            },
        );

        MockIndexerProvider { pools, tokens }
    }
